...
public class TwAirApplication {
	...
	public String search(@ModelAttribute(value="searchForm") SearchForm searchForm, Model model) throws Exception {
		...
		try {
			...
			List<ResultFlight> result = new ArrayList<>(matchingFlights.getFlightList().size());
			for (Flight flight : matchingFlights.getFlightList()) {
				ResultFlight res = new ResultFlight(flight, searchForm.getClassType(), searchForm.getNumberSeats());
				res.calculateCost();
				result.add(res);
			}
